...
from torch.utils.data import DataLoader, Subse
...
file_path = os.path.join(base_path,"datasets_indices/NewScenario/train_indices_tasks.txt")
...
file_path = os.path.join(base_path,"datasets_indices/NewScenario/val_indices_tasks.txt")
...
file_path = os.path.join(base_path,"datasets_indices/NewScenario/test_indices_tasks.txt")
...
for p in range(len(test_datasets_joint)):
    ...
    state_dict = torch.load(os.path.join(base_path,'/home/marina/continual_fairness/models/model_DER_task{0}_epoch{1}.pth'.format(p,10)))
