...
from src.training.utils import train_model_lwf_replay
...
for p in range(len(train_dataloaders)):
    old_targets = {}
    #dictionary containing the outputs of the teacher model on the new task, and on the replay samples for the new task
    if p>0:
        compute_old_targets_replay(old_targets, model, train_dataloaders, p, device, new_replayed_datasets)
    ...
    for epoch in range(num_epochs):
        ...
        train_model_lwf_replay(train_dataloaders, tasks_labels, p, tasks_labels_nih, new_replayed_datasets, model,
                               criterion, device, optimizer, epoch, old_targets)
